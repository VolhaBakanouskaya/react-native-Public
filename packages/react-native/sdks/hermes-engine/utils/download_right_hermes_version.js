...
function shouldDownloadRightHermesVersion(configuration) {
    ...
    if (!fs.existsSync && configuration === "Debug") {
        console.log('File does not exists, but Debug configuration. No need to download a new build of Hermes!');
        return false;
    }
...
const argv = yargs
    .option('c', {
        alias: 'configuration',
        description: 'Configuration to use to download the right Hermes version. Allowed values are "Debug" and "Release".',
    })
    .option('r', {
        alias: 'reactNativeVersion',
        description: 'The Version of React Native associated with the Hermes tarball.',
    })
    .usage('Usage: $0 -c Debug -r <version>')
    .argv;
